  ...
  const generateDevolucionHTML = (devolucion) => {
    ...
    const total = devolucion.total || 
      devolucion.items.reduce((sum, item) => {
        return sum + (parseFloat(item.precio_unitario) * parseFloat(item.cantidadDevolucion));
      }, 0);
      ...
      <Card style={styles.card}>
        ...
        <Card.Content>
            ...
            <Card key={index} style={styles.itemCard}>
              <Card.Content>
                <Text style={styles.productoNombre}>
                  {item.producto ? item.producto.nombre : 'Producto no especificado'}
                </Text>
          ...
          <View style={styles.totalContainer}>
            ...
            <Text style={styles.totalValue}>L. {parseFloat(devolucion.total).toFixed(2)}</Text>
          </View>
        </Card.Content>
