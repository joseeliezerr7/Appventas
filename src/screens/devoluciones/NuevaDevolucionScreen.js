...
import { ActivityIndicator, Button, Card, Checkbox, Divider, Text, TextInput } from 'react-native-paper';
...
const NuevaDevolucionScreen = () => {
  ...
  const cargarDetallesVenta = useCallback(async () => {
    try {
      ...
      const detallesVenta = await api.getVenta(venta.id);
      console.log('Detalles de venta recibidos:', detallesVenta);
      
      if (detallesVenta && detallesVenta.items) {
        // Asegurarse de que cada item tenga la información completa del producto y unidades
        const itemsConProductos = detallesVenta.items.map(item => {
          console.log('Item de venta recibido:', item);
          return {
            ...item,
            producto: {
              id: item.producto_id,
              nombre: item.producto_nombre || 'Producto sin nombre',
              codigo: item.producto_codigo || `PROD-${item.producto_id}`
            },
            // Guardar también a nivel de item para facilitar acceso
            producto_codigo: item.producto_codigo || `PROD-${item.producto_id}`,
            // Preservar información de la unidad
            unidad_id: item.unidad_id,
            unidad_nombre: item.unidad_nombre || 'Unidad'
          };
        });
        
        setVentaDetalles(itemsConProductos);
        setVentaCompleta(detallesVenta);
      } else {
        setVentaDetalles([]);
  ...
  const generateDevolucionHTML = (devolucionData) => {
    ...
    return `
      ...
      <body>
        <div class="header">
          <h2>DEVOLUCIÓN</h2>
          <p>Venta #${devolucionData.venta_id || 'N/A'}</p>
          <p>Fecha: ${devolucionData.fecha || 'N/A'}</p>
          <p>Hora: ${devolucionData.hora || ''}</p>
        </div>
        
        <div class="info-row">
          <span>Cliente:</span>
          <span>${devolucionData.cliente?.nombre || 'Cliente sin nombre'}</span>
        </div>
  ...
  const handleSubmit = async () => {
    if (itemsSeleccionados.length === 0) {
      Alert.alert('Error', 'Debe seleccionar al menos un producto para devolver');
      return;
    }
    ...
    // Usar ventaCompleta para obtener información adicional si está disponible
    const clienteInfo = ventaCompleta?.cliente || { nombre: venta.cliente?.nombre || 'Cliente sin nombre' };
    ...
    try {
      ...
      let usuario_id = null;
      try {
        const userData = await AsyncStorage.getItem('user'); // Clave correcta según AuthContext.js
        if (userData) {
          const parsedUserData = JSON.parse(userData);
          usuario_id = parsedUserData.id;
          console.log('Usuario ID obtenido:', usuario_id);
        } else {
          console.warn('No se encontró información del usuario en AsyncStorage');
        }
      } catch (error) {
        console.error('Error al obtener usuario_id de AsyncStorage:', error);
      }
      ...
      if (isNaN(devolucionData.total)) {
        devolucionData.total = 0; // Asegurarnos que total sea un número válido
      }
    
      // Verificar que todos los datos requeridos estén presentes
      console.log('Enviando devolución a la API:', JSON.stringify(devolucionData, null, 2));
      console.log('venta_id:', devolucionData.venta_id, 'total:', devolucionData.total, 'usuario_id:', devolucionData.usuario_id);
      console.log('Items seleccionados:', itemsSeleccionados.length);
      
      // Verificar que cada item tenga toda la información necesaria
      itemsSeleccionados.forEach((item, index) => {
        console.log(`Item ${index}: producto_id=${item.producto_id || item.producto?.id}, cantidad=${item.cantidadDevolucion}`);
        console.log(`  Unidad: id=${item.unidad_id}, nombre=${item.unidad_nombre || 'N/A'}`);
        console.log(`  Producto: nombre=${item.producto_nombre || item.producto?.nombre || 'N/A'}, codigo=${item.producto_codigo || item.producto?.codigo || 'N/A'}`);
      });
    ...
    <ScrollView style={styles.container}>
      {loading ? (
        <View style={styles.loadingContainer}>
          ...
          <Text style={styles.loadingText}>Cargando detalles de venta...</Text>
        </View>
      ) : generatingPDF ? (
        <View style={styles.loadingContainer}>
          <ActivityIndicator size="large" color="#007bff" />
          ...
          <Card style={styles.card}>
            ...
            <Card.Content>
              <View style={styles.infoRow}>
                ...
                <Text style={styles.infoValue}>{venta.fecha || 'Fecha no disponible'}</Text>
              </View>
              
              <View style={styles.infoRow}>
                <Text style={styles.infoLabel}>Cliente:</Text>
                <Text style={styles.infoValue}>{venta.cliente?.nombre || 'Cliente sin nombre'}</Text>
              </View>
              
              <View style={styles.infoRow}>
                <Text style={styles.infoLabel}>Total Venta:</Text>
                <Text style={styles.infoValue}>L. {parseFloat(venta.total || 0).toFixed(2)}</Text>
              </View>
