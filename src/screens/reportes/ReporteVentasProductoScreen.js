...
const ReporteVentasProductoScreen = () => {
  ...
  const [sortField, setSortField] = useState('total_ingresos');
  ...
  const exportarDatos = async (formato) => {
    try {
      ...
      const headers = ['Código', 'Producto', 'Cantidad', 'Ingresos', 'Ventas', 'Precio Prom.'];
      const title = `Reporte de Ventas por Producto - ${periodo || 'Este mes'}`;
      const filename = 'ventas_por_producto';
  ...
  const calcularTotales = () => {
    if (!filteredData || !Array.isArray(filteredData) || filteredData.length === 0) {
      return { totalIngresos: 0, totalCantidad: 0, totalVentas: 0 };
    }
    
    return filteredData.reduce((acc, item) => ({
      totalIngresos: acc.totalIngresos + parseFloat(item?.total_ingresos || 0),
      totalCantidad: acc.totalCantidad + parseInt(item?.total_cantidad_vendida || 0),
      totalVentas: acc.totalVentas + parseInt(item?.numero_ventas || 0)
    }), { totalIngresos: 0, totalCantidad: 0, totalVentas: 0 });
  };
        ...
        <Card.Content>
          ...
          <View style={styles.resumenGrid}>
            <View style={styles.resumenItem}>
              <Text style={styles.resumenValue}>{formatCurrency(totales.totalIngresos)}</Text>
              <Text style={styles.resumenLabel}>Total Ingresos</Text>
            </View>
            <View style={styles.resumenItem}>
              <Text style={styles.resumenValue}>{totales.totalCantidad}</Text>
              <Text style={styles.resumenLabel}>Unidades Vendidas</Text>
            </View>
            <View style={styles.resumenItem}>
      ...
      <ScrollView horizontal style={styles.tableContainer}>
        <DataTable style={styles.dataTable}>
          <DataTable.Header>
            ...
            <DataTable.Title
              numeric
              sortDirection={sortField === 'total_cantidad_vendida' ? sortDirection : undefined}
              onPress={() => ordenarDatos('total_cantidad_vendida')}
            >
              Cantidad
            </DataTable.Title>
            <DataTable.Title
              numeric
              sortDirection={sortField === 'total_ingresos' ? sortDirection : undefined}
              onPress={() => ordenarDatos('total_ingresos')}
            >
              Ingresos
            </DataTable.Title>
            <DataTable.Title
            ...
            <DataTable.Title
              numeric
              sortDirection={sortField === 'precio_promedio' ? sortDirection : undefined}
              onPress={() => ordenarDatos('precio_promedio')}
            >
              Precio Prom.
            </DataTable.Title>
          </DataTable.Header>
          ...
          {(filteredData || []).map((item, index) => (
            <DataTable.Row key={item?.id || index}>
              ...
              <DataTable.Cell numeric>
                {item?.total_cantidad_vendida || 0} {item?.unidad_principal || ''}
              </DataTable.Cell>
              <DataTable.Cell numeric>
              ...
              <DataTable.Cell numeric>
                {formatCurrency(item?.precio_promedio || 0)}
              </DataTable.Cell>
            </DataTable.Row>
          ))}
        </DataTable>
      ...
      {(!filteredData || filteredData.length === 0) && !loading && (
        <View style={styles.emptyContainer}>
          ...
          <Text style={styles.emptySubtext}>
            Intenta cambiar el período o los filtros de búsqueda
          </Text>
        </View>
      )}
...
export default ReporteVentasProductoScreen;